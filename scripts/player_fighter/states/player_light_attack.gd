...
#func enter() -> void:
	#active_player.velocity.x = 0
		...
		#active_player.add_to_attack_chain(active_player.light_atk_anims[active_player.light_atk_anim_index])
	#elif !active_player.attack_chain_active or active_player.attack_chain.is_empty():
		#active_player.add_to_attack_chain(active_player.light_atk_anims[0])
		#active_player.animations.play(active_player.light_atk_anims[0])
#
...
#func process_input(event: InputEvent) -> State:
			...
			#if event.is_action_pressed(active_player.light_atk_key):
