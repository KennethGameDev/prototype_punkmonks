...
func enter() -> void:
	...
	if fighter.is_light_attack_anim_playing():
		fighter.add_to_attack_chain(fighter.heavy_atk_anims[fighter.heavy_atk_anim_index])
	if !fighter.attack_chain_active or fighter.attack_chain.is_empty():
		fighter.add_to_attack_chain(fighter.heavy_atk_anims[0])
		fighter.animations.play(fighter.heavy_atk_anims[0])
...
func process_input(event: InputEvent) -> State:
	if event.is_pressed():
		if fighter.is_heavy_attack_anim_playing():
			if event.is_action_pressed(fighter.heavy_atk_key):
